<?php
		...
		$cg_ThankYouComment = $_REQUEST['action9'];
  ...
  if (empty($Comment)) {
   ...
   if ($unix-5 < $Timestamp){
    $error = true;
    $errortext .= "<br/<br/><b>You filled out the form to fast. Plz wait longer then 5 seconds.</b><br>";
  }
...
// Gecachte Gallery Files werden aktualisiert
			...
			$cachefilesGallery = dirname(__FILE__).'/../../../uploads/contest-gallery/gallery-id-'.$galeryID.'/cache/gallery/';
...
// Gecachte Site Files werden gel�scht

			$codedPictureId = ($pictureID+8)*2+100000; // Verschl�sselte ID ermitteln. Gecachte Sites sind mit verschl�sselter ID gespeichert.
					
			//$cachefilesGallery = dirname(__FILE__).'/../../../uploads/contest-gallery/gallery-id-'.$galeryID.'/cache/gallery/';
			$cachefilesSites = dirname(__FILE__).'/../../../uploads/contest-gallery/gallery-id-'.$galeryID.'/cache/sites/id-'.$codedPictureId.'*';
						
			
			// gecachte Seiten l�schen
...
// Gecachte Site Files werden gel�scht --- ENDE
...
<?php
    ...
    echo "<div id='cg_comment_success'><br><br>$cg_ThankYouComment</div>"; 
...
// Sprung zur Meldung ob Formulardaten erfolgreich �bermittelt wurden oder nicht
location.href = "#cg_comment_success";
